...
# qsub -l walltime=72:0:0 -l select=1:ncpus=8:mem=200gb:scratch_local=200gb -m abe Amblycera_12_positions_metacentrum.sh
# In this case, maximal running time is 3 days - 72 hours (72:0:0), it will run on one computer (select=1), use 8 CPU threads (ncpus=8), up to 24 GB of RAM (mem=24gb) and send email when it aborts, begins and completes/ends

# Set data directories
WORKDIR="12_positions" # Or something else
DATADIR="/storage/praha1/home/tom_zoo" # Or other storage

# There is directory /storage/praha1/home/tom_zoo/12_positions (in this case) containing all the data needed for calculations
...
# In this case (for example) IQ-TREE

module load iqtree

# Launch the analysis - run baseml
# Note Parallel will distribute task among 8 CPU threads (-j 8), so that qsub must in this case contain select=1:ncpus=8
iqtree2 -s Amblycera_12_positions.fasta -p partitions.txt -B 1000 -T 8 -m TESTNEWMERGE -rclusterf 10
